document.addEventListener('DOMContentLoaded', () => {
    ...
    function initMusicSystem() {
        for (let i = 1; i <= 11; i++) {
            let num = i < 10 ? '0' + i : i;
            playlist.push(`audios/${num}.mp3`);
        }
        shuffleArray(playlist);
        audioPlayer.volume = 0.05; // Volumen inicial 15%
        ...
        // "Unlocker" Global: Escucha el primer clic en TODO el documento (fase de captura)
        // Esto garantiza que el audio inicie incluso si el clic es en el tutorial o fondo vacío
    ...
    function playTrack(index) {
        ...
        if (audioPlayer.src.indexOf(src) === -1) {
             audioPlayer.src = src;
        }
        ...
        audioPlayer.play()
            .then(() => {
                updateMusicUI(true);
                musicStatusText.textContent = `Reproduciendo Pista ${index + 1}`;
            })
            .catch(error => {
    ...
    function updateMusicUI(isPlaying) {
        ...
        if (isPlaying) {
            ...
            musicStatusText.textContent = "Pausado";
        }
    }
    ...
    function setupEventListeners() {
        ...
        musicMenuBtn.addEventListener('click', (e) => {
            ...
            musicMenuBtn.classList.toggle('active');
        });
        ...
        document.addEventListener('click', (e) => {
            if (!musicWrapper.contains(e.target)) {
                ...
                musicMenuBtn.classList.remove('active');
            }
        });
